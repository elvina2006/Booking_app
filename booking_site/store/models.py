...
class Hotel(models.Model):
    ...
    hotel_stars = models.PositiveSmallIntegerField(validators=[MinValueValidator(1), MaxValueValidator(6)])
...
class Room(models.Model):
    ...
    TYPE_CHOICES = (
        ('люкс', 'люкс'),
        ('эсемейный', 'эсемейный'),
        ('одномесный', 'одномесный'),
        ('двухмесный', 'двухмесный')
    )
...
class Booking(models.Model):
    ...
    room_hotel = models.ForeignKey(Room, on_delete=models.CASCADE)
